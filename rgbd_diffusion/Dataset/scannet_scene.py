...
class ScanNetScene(Dataset):
    def __init__(self,
                 path,
                 # num of views per scene for UNet compute;
                 # set to None to load all
                 num_views_sample=4,
                 # maximum length of sequence; set to None to load all
                 max_len_seq=64,
                 img_size=128,
                 normalize=True,
        ...
        sorted_id = unstructured_to_structured(number_id, np.dtype(
        ...
        if isinstance(subset_indices, (tuple, list)):
            ...
            # training set has more data
            if subset_indices > len(file_path) / 2:
    ...
    def __getitem__(self, ind_chunk):
        ...
        if self.reverse_frames and torch.rand(1) < 0.5:
            views_indices = list(reversed(views_indices)
                                 )  # reversed time order
        if isinstance(self.max_len_seq, int) and \
                len(views_indices) > self.max_len_seq:
            st = torch.randint(0, len(views_indices) -
                               self.max_len_seq + 1, size=(1,)).item()
            views_indices = views_indices[st:(
                st + self.max_len_seq)]  # randomly crop
        #
        ...
        for ind_view in views_indices:
            ...
            if self.normalize:
                ...
                img_rgbd = (
                    img_rgbd - mean[:, None, None]) / std[:, None, None]
        ...
        rgbd = F.interpolate(
            ...
            **dict(align_corners=False) if self.inter_mode == "bilinear"
            else {}
        )
        #
...
def collate_fn(data):
    ...
    for i, x in enumerate(num_views_lst):
        vinds = view_index[(view_index >= start) & (
            view_index < start + x)].tolist()
        index_range.extend([i, start, vind + 1] for vind in vinds)
        start += x
