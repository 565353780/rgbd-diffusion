...
# total batch size (sum of all processes)
BATCH_SIZE = BATCH_SIZE_PER_GPU * num_rank
...
class Render(object):
    ...
    def render_many(self, pack_meshes, pack_cameras, res=128):
        ...
        for ind_mesh, (f_st, f_cnt, v_st, v_cnt) in enumerate(ind_group):
            ...
            # TODO maybe `any` is better, but compute of `msk_v` is a bit tricky
        ...
        rast, _ = dr.rasterize(self.ctx_render,
                               ...
                               ranges=ind_group[:, :2].to(
                                   torch.int32).contiguous(),  # has already on CPU
                               )
        # attribute
...
class Model(nn.Module, Render):
    ...
    def render_views(self,
                     # source views (e.g. previous views), shape == (B, N, C, H, W)
                     rgbd_src,
                     # camera params of source views, shape == (B, N, 3, 3/4)
                     c2w_src,
                     # project onto this camera view, shape == (B, 3, 3/4)
                     c2w_dst,
                     ):
...
optimizer = torch.optim.AdamW(
    model.parameters(), weight_decay=0.01, lr=LR_INIT)
